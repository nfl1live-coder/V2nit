...
const App = () => {
  ...
  const { handleLogin, handleRegister, handleGoogleLogin, processGoogleRedirect, handleLogout, handleUpdateProfile } = auth;
  ...
  useEffect(() => {
    const checkRedirect = async () => {
      try {
        ...
        console.error('Google redirect check failed:', error);
      }
    };
    checkRedirect();
